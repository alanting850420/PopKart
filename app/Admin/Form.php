...
class Form
{
    ...
    public function update($id)
    {
        ...
        if(array_key_exists('_file_del_', $data)) {
            $file_del = $data['_file_del_'];
            unset($data['_file_del_']);
        }
        ...
        if(!is_null($file_del)){
            foreach ($file_del as $del){
                $del = explode(",",$del);
                if(!array_key_exists($del[0], $data))
                    $data = array_merge($data, array($del[0] => []));
                if(!array_key_exists('_file_del_', $data[$del[0]]))
                    $data[$del[0]] = array_merge($data[$del[0]], array('_file_del_' => []));
                $data[$del[0]]['_file_del_'][] = $del[1];
            }
        }
...
//        static::doNotSnakeAttributes($this->model);
    ...
    public static function registerBuiltinFields()
    {
        $map = [
            ...
            'url'            => \App\Admin\Form\Field\Url::class,
            ...
            'listbox'        => \App\Admin\Form\Field\Listbox::class,
        ];
